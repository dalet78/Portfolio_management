...
class TradingLoopManager:
    ...
    def _process_ticker(self, ticker, strategy_name, check_signals, bar_size, sl_percent, tp_percent):
        ...
        self._place_trade_and_register(ticker, signal, result, df, strategy_name, sl_percent, tp_percent)
    ...
    def _check_and_update_trailing_sl(self):
        for stock, trade in self.trade_tracker.open_positions.items():
            ...
            current_price, contract = self.trade_tracker.get_cached_price(stock)
            if current_price is None or contract is None:
                continue
...
class TradeTracker:
    ...
    def update_price_cache(self, ib, stocks):
        self.price_cache.clear()
        contracts = []
        for stock in stocks:
            try:
                contract = ib.get_contract(stock)
                if contract:
                    contracts.append((stock, contract))
            except Exception as e:
                LoggerSingleton.get_logger().log(f"❌ Failed to get contract for {stock}: {e}", stock=stock, level="error")  # ✅ Added robustness
        for stock, contract in contracts:
            try:
                ticker_data = ib.ib.reqMktData(contract, '', False, False)
                ib.ib.sleep(0.2)
                last_price = ticker_data.last if ticker_data.last else ticker_data.close
                self.price_cache[stock] = (last_price, contract)
            except Exception as e:
                LoggerSingleton.get_logger().log(f"❌ Failed to update price for {stock}: {e}", stock=stock, level="error")  # ✅ Added robustness

    def get_cached_price(self, stock):
        return self.price_cache.get(stock, (None, None))
