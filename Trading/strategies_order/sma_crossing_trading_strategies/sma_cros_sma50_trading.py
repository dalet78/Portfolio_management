...
log = Logger("/home/dp/PycharmProjects/Portfolio_management/Portfolio_management/Reports/logs/sma_cross_sma50.log")
...
def check_signals(df, stock):
    """Analyze SMA crossovers and return a BUY/SELL signal considering SMA50 as trend filter."""
    ...
    try:
        ...
        df['SMA50'] = df['close'].rolling(50).mean()  # Trend filter
        ...
        log.log(f"SMA5: {sma1}, SMA20: {sma2}, SMA50: {sma50} | Previous SMA5: {prev_sma1}, SMA20: {prev_sma2}", stock=stock)

        # Bullish crossover above SMA50
        if sma1 > sma2 and prev_sma1 <= prev_sma2 and candle_direction == 'bullish' and df['close'].iloc[-1] > sma50:
            log.log("BUY signal detected (above SMA50)", stock=stock, level="info")
            return 'BUY'
        # Bearish crossover below SMA50
        elif sma1 < sma2 and prev_sma1 >= prev_sma2 and candle_direction == 'bearish' and df['close'].iloc[-1] < sma50:
            log.log("SELL signal detected (below SMA50)", stock=stock, level="info")
            return 'SELL'
...
def trading_loop():
    ...
    tickers_list = get_stock_list("SMA_Cross_sma50")
