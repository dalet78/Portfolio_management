...
from libs.indicators.lagging_indicator_handler import LaggingIndicators
...
class HOLCStrategy(Strategy):
    ...
    def add_macd(self, short_period=12, long_period=26, signal=9):
        ...
        """
        self.data.df['MACD'] = self.data.df['Close'].ewm(span=short_period, adjust=False).mean() - \
                            self.data.df['Close'].ewm(span=long_period, adjust=False).mean()
        self.data.df['MACD_Signal'] = self.data.df['MACD'].ewm(span=signal, adjust=False).mean()
    ...
    def next(self):
        ...
        macd_line = self.data['MACD'][-1]
        signal_line = self.data['MACD_Signal'][-1]
        macd_line_prev = self.data['MACD'][-2]
        signal_line_prev = self.data['MACD_Signal'][-2]
        ...
        if self.position and current_time >= exit_time:
            ...
            # MACD cross logic
            is_macd_buy_crossover = macd_line > signal_line and macd_line_prev <= signal_line_prev
            is_macd_sell_crossunder = macd_line < signal_line and macd_line_prev >= signal_line_prev

            if is_macd_buy_crossover and candle_direction == "bullish":
                stop_loss = self.data.Low[-1] - 0.15
                take_profit = self.data.Close[-1] + 0.30
                self.buy(sl=stop_loss, tp=take_profit)

            elif is_macd_sell_crossunder and candle_direction == "bearish":
