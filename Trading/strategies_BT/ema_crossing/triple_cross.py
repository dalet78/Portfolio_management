...
class HOLCStrategy(Strategy):
    ...
    def init(self):
        ...
        self.ema9 = self.I(EMA, price, 9)  # EMA9
        self.ema21 = self.I(EMA, price, 21)  # EMA21
        self.ema50 = self.I(EMA, price, 50)  # EMA50
    ...
    def next(self):
        ...
        # Chiudi tutte le posizioni alla fine della giornata
        if self.position and current_time >= EXIT_TIME:
            self.position.close()
            self.last_trade_date = None
            return  # Evita di continuare a elaborare entry dopo l'orario di uscita

        # Candele precedenti
        ema9_prev2, ema9_prev1 = self.ema9[-3], self.ema9[-2]
        ema21_prev2, ema21_prev1 = self.ema21[-3], self.ema21[-2]
        ema50_prev2, ema50_prev1 = self.ema50[-3], self.ema50[-2]

        next_candle_open = self.data.Open[-1]  # ENTRY PRICE all'apertura della candela successiva

        # Controlla se abbiamo già aperto un trade oggi
        if self.last_trade_date == current_date:
            return

        # **Bullish Crossover**: Tutte le EMA si incrociano verso l'alto in 2 candele
        if (
            ema9_prev2 < ema21_prev2 and ema9_prev1 > ema21_prev1 and  # EMA9 sopra EMA21
            ema21_prev2 < ema50_prev2 and ema21_prev1 > ema50_prev1 and  # EMA21 sopra EMA50
            self.data.Close[-1] > self.ema50[-1]  # Prezzo sopra EMA50
        ):
            entry_price = next_candle_open
            stop_loss = entry_price * (1 - SL_PERCENT)
            take_profit = entry_price * (1 + TP_PERCENT)
            self.buy(sl=stop_loss, tp=take_profit)  # Uso sl e tp direttamente

        # **Bearish Crossover**: Tutte le EMA si incrociano verso il basso in 2 candele
        elif (
            ema9_prev2 > ema21_prev2 and ema9_prev1 < ema21_prev1 and  # EMA9 sotto EMA21
            ema21_prev2 > ema50_prev2 and ema21_prev1 < ema50_prev1 and  # EMA21 sotto EMA50
            self.data.Close[-1] < self.ema50[-1]  # Prezzo sotto EMA50
        ):
            entry_price = next_candle_open
            stop_loss = entry_price * (1 + SL_PERCENT)
            take_profit = entry_price * (1 - TP_PERCENT)
            self.sell(sl=stop_loss, tp=take_profit)  # Uso sl e tp direttamente
...
def ema_cross_trading(index = "SP500"):
    ...
    file_report = report.save_report(filename=f"{index}_ema_triple_cross_stock")
    return file_report
