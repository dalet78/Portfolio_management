...
class VwapDiffStrategy(Strategy):
    last_trade_date = None
    entry_time = None
    reduced_tp_price = None  # Nuovo TP al 1%
    ...
    def next(self):
        ...
        if self.position and time_only >= EXIT_TIME:
            self.position.close()
            self.last_trade_date = None
            self.entry_time = None
            self.reduced_tp_price = None
            return

        # Gestione take profit modificato dopo 1 ora
        if self.position and self.entry_time:
            if current_time - self.entry_time >= timedelta(hours=1):
                if not self.reduced_tp_price:
                    if self.position.is_long:
                        self.reduced_tp_price = self.position.entry_price * 1.01
                    elif self.position.is_short:
                        self.reduced_tp_price = self.position.entry_price * 0.99

                # Chiudi se nuovo TP raggiunto
                if self.position.is_long and price >= self.reduced_tp_price:
                    self.position.close()
                elif self.position.is_short and price <= self.reduced_tp_price:
                    self.position.close()

        # Apertura di nuove posizioni
        elif (ENTRY_START_TIME <= time_only <= ENTRY_END_TIME and not self.position and
            ...
            if self.signal[-1] == 1:
                stop_loss = low - (SL_PERCENT * self.data.Close[-1])
                take_profit = self.vwap[-1]
                self.buy(sl=stop_loss, tp=take_profit)

            elif self.signal[-1] == -1:
                stop_loss = high + (SL_PERCENT * self.data.Close[-1])
                take_profit = self.vwap[-1]
                self.sell(sl=stop_loss, tp=take_profit)


def vwap_diff_trading(index = "ALL"):
    global df
    ...
    if not tickers_list:
        ...
        for item in tickers_list:
            ...
            try:
                ...
                bt = Backtest(df, VwapDiffStrategy, cash=10000, exclusive_orders = True)
                stats = bt.run()
                print(stats)
...
def add_vwap_and_signal(df):
    ...
    df['numeric_signal'] = np.where(
        df['High_vwap_diff'] >= TP_PERCENT * df['Close'], -1,
        np.where(df['Low_vwap_diff'] <= -(TP_PERCENT * df['Close']), 1, 0)
    )
    return df
